...
int main(int argc, char *argv[])
{
  ...
  MPI_Datatype column;    // to send left and right columns
  MPI_Status status;
  MPI_Init(&argc, &argv);
  ...
  MPI_Type_vector(by, 1, width, MPI_DOUBLE, &column);
  ...
  P = malloc(sizeof(double*) * height);
  P[0] = malloc(sizeof(double) * height * width);
  for(i=1; i < height; i++)
   P[i] = &(P[0][i * width]);
 
  A = malloc(sizeof(double*) * height);
  A[0] = malloc(sizeof(double) * height * width);
  for(i=1; i < height; i++)
    A[i] = &(A[0][i * width]);
  ...
  if (Js > 0) {
    b = B[myi][myj-1];
    if ((myi+myj) % 2 == 0) {
      MPI_Send(&P[1][1], 1, column, b, 0, MPI_COMM_WORLD);
      MPI_Recv(&P[1][0], 1, column, b, 0, MPI_COMM_WORLD, &status);
    } else {
      MPI_Recv(&P[1][0], 1, column, b, 0, MPI_COMM_WORLD, &status);
      MPI_Send(&P[1][1], 1, column, b, 0, MPI_COMM_WORLD);
    }
  ...
  if (Je < M-1) {
    b = B[myi][myj+1];
    if ((myi+myj) % 2 == 0) {
      MPI_Send(&P[1][bx], 1, column, b, 0, MPI_COMM_WORLD);
      MPI_Recv(&P[1][bx+1], 1, column, b, 0, MPI_COMM_WORLD, &status);
    } else {
      MPI_Recv(&P[1][bx+1], 1, column, b, 0, MPI_COMM_WORLD, &status);
      MPI_Send(&P[1][bx], 1, column, b, 0, MPI_COMM_WORLD);
    }
