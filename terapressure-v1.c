...
int main(int argc, char *argv[])
{
  ...
  long n=2,  m=3;        // number of blocks nxm
  ...
  long bi, bj;           // block size in y and x direction
  ...
  if (myid==0) {
    printf("===============\n");
    printf("Terapressure v1\n");
    printf("===============\n");
    printf("Number of cells: %lu (%lu x %lu)\n", N*M, N, M);
    printf("Number of blocks: %lu (%lu x %lu)\n", n*m, n, m);
    printf("Number of processors %d\n", numprocs);
    printf("Block size: %lu (%lu x %lu)\n", bi*bj, bi, bj);
  }
